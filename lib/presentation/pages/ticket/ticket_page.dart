    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: tripId == null
            ...
            : FutureBuilder<Trip?>(
                future: RepositoryProvider.of<TripRepository>(context).getTripById(tripId),
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return const Center(child: CircularProgressIndicator());
                  }
                  if (snapshot.hasError) {
                    return const _TicketError(message: 'Error al cargar el ticket');
                  }
      ...
      bottomNavigationBar: tripId == null
          ? null
          : SafeArea(
...
class _TicketError extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colors = theme.colorScheme;
    return Center(
      child: Text(message, style: theme.textTheme.bodyLarge?.copyWith(color: colors.error)),
    );
  }
}
